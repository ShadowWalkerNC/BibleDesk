...
export function getNextChapter(bookName: string, chapter: number): { book: string, chapter: number } | null {
  ...
  const currentBook = BIBLE_BOOKS[index];
  
  if (chapter < currentBook.chapters) {
    return { book: currentBook.name, chapter: chapter + 1 };
  } else if (index < BIBLE_BOOKS.length - 1) {
    return { book: BIBLE_BOOKS[index + 1].name, chapter: 1 };
  }
