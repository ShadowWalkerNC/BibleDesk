...
export default function PrayerEscalationModal({
  ...
  useEffect(() => {
    ...
    setSelectedLevel(prayer.escalation_level === 'private' ? 'circle' : (prayer.escalation_level || 'circle'));
    setSelectedUrgency(prayer.urgency_level || 'normal');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const succeeded = await onEscalate({
        prayerId: prayer.id,
        ...
        <form onSubmit={handleSubmit} className={styles.body}>
          ...
          <div className={styles.prayerSummary}>
            ...
            <div className={styles.prayerSummaryText}>
              &ldquo;{prayer.text.length > 120 ? prayer.text.slice(0, 117) + '...' : prayer.text}&rdquo;
            </div>
          </div>
          ...
          <label className={styles.toggleRow}>
            ...
            />
            <span>Anonymize Request (hide my name; post as &ldquo;A Brother/Sister in Christ&rdquo;)</span>
          </label>
